import numpy as np
import pandas as pd
from preprocessing import flatten_signal, center_matrix, extend_all_channels, whiten
...
def deflate(w, B):
    ...
    --------
        ...
        array([-28. ,  -3.2,   6. ])
    """
    w = w - np.dot(B.T, np.dot(B, w)) # Swapped order of B and B.T to orthogonalize columns and not rows
    return w
...
def refinement(
    ...
    for iter in range(max_iter):
        ...
        peak_indices, _ = find_peaks( # Peak-finding algorithm
            s_i2, distance=l
        )
