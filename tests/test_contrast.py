...
def test_skew():
    ...
    for i in range(0, 10):
        ...
        assert np.array_equal(
            manual_calc, emg_no_deriv
        ), "Contrast function incorrectly applied."

        # Reverse calculate initial values in input
        reverse_calc = np.cbrt(emg_no_deriv[-1][-1] * 3)

        assert np.allclose(
            reverse_calc, test_arr[-1][-1]
        ), "Contrast function incorrectly applied."
        ...
        assert np.allclose(
            manual_calc_deriv, emg_deriv
        ), "First derivative not calculated correctly."

        # Reverse calculate initial values in input
        reverse_calc_der = np.sqrt(emg_deriv[-1][-1])
        abs_test_value = abs(
            test_arr[-1][-1]
        )  # Absolute because +/- is lost in back calculation

        assert np.isclose(
            reverse_calc_der, abs_test_value
        ), "Contrast function incorrectly applied."
...
def test_log_cosh():
    ...
    for i in range(0, 5):
        ...
        x_cosh = (
            1 / 2 * (np.exp(test_arr) + np.exp(-test_arr))
        )  # Manually calculate cosh(x)
        x_log = np.log(x_cosh)

        assert np.allclose(
            x_log, emg.log_cosh(test_arr)
        ), "Base contrast function incorrectly calculated."

        # Test first derivative of contrast function, tanh(x)
        x_tanh = np.sinh(x) / np.cosh(x)  # mMnually calculate tanh(x)

        assert np.allclose(
            x_tanh, emg.log_cosh(x, der=True)
        ), "First derivative contrast function incorrectly calculated."
    ...
    assert np.allclose(
        emg.log_cosh(z, der=True), z_first_deriv
...
def test_exp_sq():
    ...
    for i in range(0, 10):
        ...
        assert np.count_nonzero(exp_fx) == np.count_nonzero(
            no_deriv
        ), "Base contrast function incorrectly calculated."
        assert np.argmax(exp_fx) == np.argmax(
            no_deriv
        ), "Base contrast function incorrectly calculated."
        assert np.array_equal(
            exp_fx, no_deriv
        ...
        assert np.count_nonzero(der_fx) == np.count_nonzero(
            first_deriv
        ), "First derivative function incorrectly calculated."
        assert np.argmax(der_fx) == np.argmax(
            first_deriv
        ), "First derivative function incorrectly calculated."
        assert np.allclose(
            der_fx, first_deriv
...
def test_apply_contrast():
    ...
    # test each contrast function return correct value
    ...
    for i in fx:
        ...
        assert np.array_equal(
            no_deriv, i(test_arr)
        ), "Called function not applied properly."

        first_deriv = emg.apply_contrast(test_arr, fun=i, der=True)
        assert np.array_equal(
            first_deriv, i(test_arr, der=True)
        ), "Called function not applied properly."
