...
from emgdecompy.decomposition import initial_w_matrix, deflate, gram_schmidt, orthogonalize, peel_off, normalize, separation, silhouette_score, pnr, refinement, decomposition
...
def muap_dict_by_peak(raw, peak, mu_index=0, l=31):
    ...
    signal = shape.flatten() # TODO: MAKE SURE IT DOESNT FAIL WHEN FIRST FIRING IS BEFORE l=31

    if peak < l:
        neg_idx = abs(peak - l)
        signal[:neg_idx] = np.repeat(0, neg_idx)
    
    shape_dict[f"mu_{mu_index}"] = {"sample": sample, "signal": signal, "channel": channel_index}
    
    return shape_dict


# ## Function: channel_preset
...
def muap_plot(mu_data, mu_index, peak_data=None, l=31, peak="", method=RMSE, preset='standard'):
    """
    Parameters
    ----------
    mu_data: dict
    ...
    l: int
        One half of action potential discharge time in samples.
    peak: int:
        Index of the peak, only used for the Title of the plot.
    ...
    if peak_data:   
        ...
        err = round(err)
        plot_title = f"Peak #{peak} contribution per Channel to MU {mu_index}. RMSE = {err}"
        legend_position = alt.Legend(orient='none',
                                     title=None,
                                     legendX=400, 
                                     legendY=-40,
                                     direction='horizontal',
                                     titleAnchor='middle') # Show Legend when showing overlay
    ...
    plot = (
        ...
        .encode(
            ...
            color=alt.Color("Source", scale={"range": ["#fd3a4a", "#99a7f1"]}, legend=legend_position),
            opacity=alt.condition(selection, alt.value(1), alt.value(0.2)),
            facet=alt.Facet(
                "channel",
                columns=cols,
                spacing={'row': 0},
                header=alt.Header(titleFontSize=0, titleOrient="bottom", labelFontSize=14, labelOrient='bottom'),
                sort=sort_order
            ),
        )
        .mark_line()
...
# # Signal and IFR Plotting Function
...
def pulse_plot(pt, c_sq_mean, mu_index=None, sel_type='single'):
    ...
    for i in range(1, mu_count + 1):
        ...
        # Make those into DF:
        pulses_i = {"Pulse": pt_selected, "Strength": strength_selected, "Motor Unit": i, 'seconds': pt_selected/2048, 'Hz': hz_ms}
    ...
    chart_top_base = alt.Chart(motor_df).encode(
        alt.X('seconds:Q', 
              axis=alt.Axis(title='Time (sec)', grid=False),
             )
    ).properties(width = 1000, height = 100)

    chart_top_rate = chart_top_base.mark_point(size=30, color=color_rate).encode(
        alt.Y('Hz:Q',
              axis=alt.Axis(title='Instantaneous Firing Rate (Hz)', 
                            grid=False, 
                            format='.0f', 
                            titleColor=color_rate))
    ).add_selection(sel_interval)

    chart_top_pulse = chart_top_base.mark_bar(size=3.5, color=color_pulse, opacity=0.3).encode(
        alt.Y('Strength:Q', 
                      axis=alt.Axis(title='Signal (A.U.)', 
                                    grid=False, 
                                    format='s', 
    ...
    chart_rate = alt.Chart(motor_df).encode(
        alt.X('seconds:Q', 
              axis=alt.Axis(title='Time (sec)', grid=False),
        ...
        color=alt.condition(sel_peak, 
                            alt.value(color_rate), 
                            alt.value('lightgray'), 
                            legend=None),
        tooltip=[alt.Tooltip('Hz', format='.2f'), alt.Tooltip('seconds', format='.2f')]
        ).properties(width = 1000, height = 250).mark_point(size=30).add_selection(sel_peak).transform_filter(sel_interval)


    chart_pulse = alt.Chart(motor_df).encode(
        alt.X('seconds:Q', 
              axis=alt.Axis(title='Time (sec)', grid=False),
              scale=alt.Scale(domain=sel_interval)),
        alt.Y('Strength:Q',
              axis=alt.Axis(title='Signal (A.U.)',
                                ...
                                titleColor=color_pulse)),
        color=alt.condition(sel_peak, 
                            alt.value(color_pulse), 
                            alt.value('lightgray'), 
                            legend=None)
    ).mark_bar(size=3.5).add_selection(sel_peak).properties(
        width = 1000, height = 250).transform_filter(sel_interval)
...
widget1 = widgets.Dropdown(
    options=[i for i in range(0, len(pt))],
    value=0,
...
def select_peak(selection, mu_index, raw, shape_dict, pt):
    ...
    if not selection:
        ...
        sel = selection[0]-1; # for some reason beyond my grast these are 1-indexed
...
def remove_false_peak(decomp_results, mu_index, peak):
    ...
    decomp_results['MUPulses'][0][mu_index] = (
        np.delete(decomp_results['MUPulses'][0][mu_index],
...
def dashboard(pt, c_sq_mean, mu_index, raw, shape_dict):
    ...
    return pn.Column(vega_pane, pn.bind(select_peak, 
