...
A blind source separation algorithm can decompose raw EMG signals into several individual electrical signals that can be ascribed to a singular MU. The partner currently decomposes EMG signals using free software from @ot_bioelettronica called `OTBioLab+`. This software's graphical user interface can be seen in **figure \@ref(fig:OTBioelettronica-GUI)**. `OTBioLab+` determines the individual MUAP spike trains using a closed-source algorithm based on a paper published by @negro_muceli_castronovo_holobar_farina_2016.
