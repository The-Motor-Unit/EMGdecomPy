...
1. Build custom software that inputs the raw signals *(see Fig.1)* provided by the surface 
electrode and outputs the decomposed data.

2. Create a tool to visualize the decomposed signals.


```{r EMG Signal Example, echo=FALSE, fig.cap = "***Fig. 1** A raw EMG signal prior to decomposition. EMG readings measure the net voltage of a muscle as it relaxes and contracts over time.*", fig.align="center"}
...
1. Provide a graphical user interface (GUI) in the visualization tool for manually 
...
needs. As is, the decomposition algorithm is a robust, generalizable solution 
...
fit the specific needs of the Sensimotor Physiology Lab.
...
false positive observations from the beginning sample a decomposition, allowing 
the algorithm to integrate this new information in the rest of the decomposition.
...
fixed-point K-Means classification. 
